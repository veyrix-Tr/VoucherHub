...
import IssueVoucherModal from "./IssueVoucherModal.jsx";
  ...
  return (
    <>
      <div className="bg-white/90 dark:bg-slate-800/30 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xl">
        ...
        <div className="px-6 pt-6 flex gap-2 overflow-x-auto">
          {Object.entries(statusConfig).map(([key, { label, icon }]) => (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left font-medium cursor-pointer ${activeTab === key
        ...
        <div className="p-6">
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {current.map(v => (
                <VoucherCard key={v._id} voucher={v} role="merchant" onIssue={(v) => setSelectedIssueVoucher(v)} />
              ))}
            </div>
          )}
        </div>
      ...
      {selectedIssueVoucher && (
        <IssueVoucherModal
          voucher={selectedIssueVoucher}
          signer={signer}
          onClose={() => setSelectedIssueVoucher(null)}
          onSuccess={() => {
            setSelectedIssueVoucher(null);
            loadVouchers(); // refresh after issuing
          }}
        />
      )}
    </>
